import type {
  ...
  export interface MFRuntimeLoaderOptions {
   ...
   */
  export class MFRuntimeLoader {
    private mfInstance: ModuleFederation;
    private options: Required<Pick<MFRuntimeLoaderOptions, 'fetchImpl' | 'transformPluginManifest'>>;
    private registeredRemotes: Set<string> = new Set();
  
    constructor(options: MFRuntimeLoaderOptions = {}) {
      const {
